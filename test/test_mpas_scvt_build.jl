...
@testitem "scvt driver: external loop → dodecahedral MpasMeshData (uniform CVT)" tags = [:grid, :mpas, :scvt, :driver, :lloyd] begin
    ...
    @test sum(bg_mass) ≈ sphere_area rtol = 1.0e-12            # D1 mass = ∫ρ dA = 4πR²
    sol = scvt_lloyd_solve(V0, bg_coord, bg_mass; tol = 1.0e-10, max_iters = 200)
    @test sol.converged
    @test sol.iterations >= 1
    @test size(sol.generators) == (3, 12)
    @test all(c -> isapprox(norm(sol.generators[:, c]), R; rtol = 1.0e-12), 1:12)

    # The full driver emits the dodecahedron (dual of the icosahedron): 12 pentagons.
    mesh = build_scvt_mesh(;
        generators = V0, density = nothing, background_level = 2,
        tol = 1.0e-10, max_iters = 200
    )
    ...
    @test sum(mesh.area_cell) ≈ sphere_area rtol = 1.0e-12
    ...
    for c in 1:12
        @test norm((mesh.x_cell[c], mesh.y_cell[c], mesh.z_cell[c])) ≈ R rtol = 1.0e-12
        @test mesh.z_cell[c] ≈ R * sin(mesh.lat_cell[c]) atol = 1.0e-6 * R
        @test mesh.x_cell[c] ≈ R * cos(mesh.lat_cell[c]) * cos(mesh.lon_cell[c]) atol = 1.0e-6 * R
        @test mesh.y_cell[c] ≈ R * cos(mesh.lat_cell[c]) * sin(mesh.lon_cell[c]) atol = 1.0e-6 * R
    end
...
@testitem "scvt driver: icosahedral-dual at level 1 (pentagons + hexagons)" tags = [:grid, :mpas, :scvt, :driver] begin
    ...
    mesh = build_scvt_mesh(;
        generators = V1, density = nothing, background_level = 3,
        tol = 1.0e-9, max_iters = 300
    )
...
@testitem "scvt driver: a density function drives variable resolution" tags = [:grid, :mpas, :scvt, :driver, :lloyd] begin
    ...
    mesh_d = build_scvt_mesh(;
        generators = V0, density = (x, y, z) -> 2.0 + z,
        background_level = 2
    )
    ...
    @test sum(mesh_d.area_cell) ≈ 4 * pi * R^2 rtol = 1.0e-12
...
@testitem "scvt driver: the host loop wraps the declarative step (genuine fixed point)" tags = [:grid, :mpas, :scvt, :driver, :lloyd] begin
    ...
    sol = scvt_lloyd_solve(Vp, bg...; tol = 1.0e-10, max_iters = 500)
    @test sol.converged
    ...
    step1 = scvt_lloyd_solve(sol_unit, bg...; tol = 1.0e-14, max_iters = 1)
    @test step1.displacement < 1.0e-9

    # And the driver from the same perturbed seed still emits a valid mesh.
    mesh = build_scvt_mesh(;
        generators = Vp, density = nothing, background_level = 2,
        tol = 1.0e-10, max_iters = 500
    )
    @test ESD.check_mesh(mesh, true) === nothing
    @test sum(mesh.area_cell) ≈ 4 * pi * 6.371e6^2 rtol = 1.0e-12
end
