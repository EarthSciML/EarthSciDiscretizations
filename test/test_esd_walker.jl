...
@testitem "walker: discovers seeded rules; per-layer outcomes match the catalog ledger" begin
    ...
    # explicit elseif below. Layer-B still SKIPs (convergence fixture applicable:false,
    # face-stagger binding + ghost-width contract pending in ESS).
    # lax_friedrichs_flux_cubed_sphere_{eta,xi} sit here because their convergence
    # fixtures ship `applicable:false` (cubed_sphere ESS dispatch pending).
    # The eta canonical fixture declares `applicable:false`, so Layer-A SKIPs via
