...
array `ze`; its `zc`/`dz` templates are the cartesian ones imported under a
...
domain faces, `lev`/`lev_nodes` the current centre/interface) so an integral
over any other bounds is never lowered silently — all second order on a
stretched column), the lat-lon
