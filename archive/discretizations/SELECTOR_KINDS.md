...
|---|---|---|---|
| 1 | Selector `kind` naming: per-family (`vertical`, `latlon`, `arakawa`) or structural (`structured_1d`, `structured_2d_metric`)? | **Per-family.** Each kind names exactly one grid-family accessor; structural categories may emerge once 3+ families share a stencil but are premature today. | dsc-mzu |
...
| 3 | Metric for non-uniform vertical grids (`eta`, `theta`, `z` stretched). | **Per-cell `dz[k]` binding via `{ op: "index", args: ["dz", "$k"] }` in the coefficient AST.** The `dz` array holds the per-cell layer thickness; `"$k"` is the axis pattern variable from `applies_to.dim`. This became available once the ESS S2 per-cell metric-binding evaluator (`ess-sra`) landed. See `centered_2nd_nonuniform_vertical.json` for the canonical example. The `1d_vertical_column` canonical-pipeline runner is implemented (`test/walk_esd_tests.jl::_run_layer_b_1d_vertical_column`, `ESD/dsc-yz0m`) but binds only the uniform spacing scalar `h`; Layer-B activation for the non-uniform rule awaits a per-cell `dz` array binding in that runner. | dsc-mzu / esd-9qs |
...
`cells_on_edge`, `cells_on_cell`, …). Two structural selector kinds (per
...
- `table` — name of a connectivity array on the grid runtime. The MPAS
  family is loader-backed (`grids/mpas.schema.json` options: `loader`, `R`,
  `ghosts`, `dtype`): connectivity tables are derived from the referenced
  mesh file at bind time, so the name must match a mesh field in
  `src/grids/mpas.jl` (e.g. `cells_on_edge`, `edges_on_cell`).
...
| 12 | Per-cell metric bindings (`cos_lat` for latlon, `dz_k` for stretched vertical). | **Per-family grid-accessor binding**. Per-cell metrics are evaluated against the grid accessor at stencil-application time — distinct from scalar bindings (`R`, `dlon`, `dlat`) which are constant across the sweep. In rule `coeff` ASTs, two binding styles exist: per-cell metric *arrays* are indexed explicitly as `{ op: "index", args: ["metric_name", "$axis"] }` (see `upwind_1st_nonuniform.json` for `dx[k]`; `centered_2nd_nonuniform_vertical.json` for `dz[k]`), while per-row scalar metrics appear as **bare symbols** that the grid accessor rebinds at each application point (see `centered_2nd_uniform_latlon.json`, where `cos_lat` is a bare symbol inside `{"op": "*", "args": [2, "R", "cos_lat", "dlon"]}` — no `index` op). The ESS S2 per-cell evaluator (`ess-sra`) activated these bindings; `centered_2nd_uniform_latlon`'s canonical Layer-A fixture is now active (esd-9qs). | dsc-8ad / esd-9qs |

The cubed-sphere selector decisions that previously lived here (in-panel
offset encoding, D-grid staggering, FV3 super-grid metric bindings) were
removed when the cubed-sphere grid was retired — the grid was only ~90%
expressible as declarative rules and is no longer supported.
...
Adding a new family (e.g. `latlon`, `mpas`):

1. Pick a per-family kind name in lowercase snake_case.
2. Decide the canonical axis names (`(:lon, :lat)`, …).
...
- `../docs/rule-catalog-roadmap.md` — manifest of rules to port across families.
