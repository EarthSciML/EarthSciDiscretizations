---
title: "mixed_deriv_2nd_nonuniform"
slug: "mixed_deriv_2nd_nonuniform"
families: "finite_difference"
grid_families: "cartesian"
rule_kinds: "scheme"
accuracy: "O(dx_i*dy_j)"
applies_to: "grad({'op': 'grad', 'args': ['$u'], 'dim': '$y'}), dim=$x"
rule_path: "discretizations/finite_difference/mixed_deriv_2nd_nonuniform.json"
description: "Auto-generated catalog entry for the mixed_deriv_2nd_nonuniform rule (finite_difference, grid family cartesian). The discrete operator is pretty-printed directly from the rule's replacement AST."
---
...
```text
out[$x, $y] = (index($u, $x + 1, $y + 1) − index($u, $x + 1, $y − 1) − (index($u, $x − 1, $y + 1) − index($u, $x − 1, $y − 1)))/((index(dx, $x) + index(dx, $x + 1))·(index(dy, $y) + index(dy, $y + 1)))
```

Source: [`discretizations/finite_difference/mixed_deriv_2nd_nonuniform.json`]({{< param repoURL >}}/blob/main/discretizations/finite_difference/mixed_deriv_2nd_nonuniform.json).
