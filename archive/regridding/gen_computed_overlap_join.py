...
def main():
    ...
    # --- A_j_w: build-once target denominator Σ_i A_ij over the SAME broad-phase
    #     bin-skolem candidate set (join.on [[src_bin,tgt_bin]]) + sub-atol sliver
    #     filter as the numerator (eq 5), so denominator and numerator contract over
    #     IDENTICAL (i,j) pairs and the partition of unity holds exactly. A plain
    #     dense Σ_i is WRONG at scale: a spherical clip emits a ~1e-9 sliver for
    #     EDGE-ADJACENT (non-overlapping) cells, which the broad-phase join (the
    #     neighbour lands in a different bin) and the sliver filter both exclude from
    #     the numerator — but a dense sum keeps in the denominator, leaking mass and
    #     biasing F_tgt by O(sliver/A_j). The inner aggregate contracts over i only;
    #     the output index j is bound by the outer arrayop (so tgt_bin resolves to j).
    A_jw_expr = arrayop(["j"], [("j", "tgt_cells")],
                        {"op": "aggregate", "semiring": "sum_product", "output_idx": [],
                         ...
                         "filter": {"op": ">", "args": [ix("A_ij", "i", "j"), "atol"]},
    ...
    md["description"] = (
        ...
        "build-once row-sum Σ_i A_ij over the SAME broad-phase bin-skolem candidate set + sub-atol "
        "sliver filter as the apply numerator (no host dst_areas), so the partition of unity holds "
        "exactly at scale (a dense Σ_i would leak the ~1e-9 edge-adjacent slivers a spherical clip "
        "emits, biasing F_tgt). The broad-phase bin-skolem floor->skolem->distinct->equijoin candidate "
        "set and the sub-atol sliver filter are unchanged from the "
