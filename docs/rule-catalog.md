...
| `lax_friedrichs_flux` | `finite_volume` | `scheme` | §7 | `src/operators/flux_1d.jl`, `transport_2d.jl`; mirrored as `discretizations/finite_volume/lax_friedrichs_flux.json` | P2 | atmos.dyn (reference / debugging) | none for the in-panel Cartesian core; the cubed-sphere wrapper depends on schema follow-ups (boundary_policy + face-stagger / time-varying bindings) tracked off dsc-35x | S | looks_ok | `discretizations/finite_volume/lax_friedrichs_flux.json` | — | Now in repo as the 2-point cartesian flux stencil F_{i+1/2} = max(c,0)·q_i + min(c,0)·q_{i+1} with face-Courant `$c` carried as a per-face binding (analogous to limiter `$r`); upwind branch encoded directly via `abs`. Cubed-sphere wrapper now lives at `lax_friedrichs_flux_cubed_sphere_xi.json` / `_eta.json` (sibling-file pair following the FV3 sinsg precedent). PPM-based flux-form transport sibling now mirrored as `flux_1d_ppm.json` (dsc-r1i); imperative `flux_1d_ppm!` / `flux_1d_ppm_arrayop` retained in `src/operators/flux_1d.jl` for `transport_2d` callers. |
| `lax_friedrichs_flux_cubed_sphere_xi` | `finite_volume` | `scheme` | §7 | `src/operators/flux_1d.jl` (`flux_1d` ArrayOp on CubedSphereGrid, dim=:xi branch); mirrored as `discretizations/finite_volume/lax_friedrichs_flux_cubed_sphere_xi.json` | P2 | atmos.dyn (reference / debugging on cubed sphere), atmos.chem.transport (oracle scheme) | none — uses already-landed face-stagger schema infra (cubed_sphere_stagger enum from dsc-247). Boundary_policy metadata for grid.dist_xi_bnd is encapsulated in the accessor's Courant precomputation rather than expressed in the rule schema (follow-up tracked off dsc-0fd). | S | looks_ok | `discretizations/finite_volume/lax_friedrichs_flux_cubed_sphere_xi.json` | `lax_friedrichs_flux`, `cubed_sphere_panel_connectivity` | ξ-direction cubed-sphere face-stagger wrapper for the LF flux. Two-point stencil at offsets {-1, 0} along ξ at cell_center stagger; emits at u_edge with face-staggered Courant `$c` consumed via the `reads` block at u_edge offset 0. Closed-form upwind-blended F = (c+|c|)/2·q_west + (c-|c|)/2·q_east mirrors the Cartesian core coefficient-for-coefficient — only selector kind / stagger differs. Cross-panel ghost extension and panel-boundary distance handling for the Courant precomputation live in the cubed_sphere grid accessor (`src/grids/panel_connectivity.jl`, `_get_courant_xi`) per SELECTOR_KINDS.md decision #13. Layer-A canonical fixture pins per-face flux equivalence on c4 vs `flux_1d`; Layer-B convergence is `applicable: false` (face-stagger MMS-transport harness pending, shared with `transport_2d` and the limiter Layer-B′); Layer-C integration stub for Williamson-1 cosine bell behind ESD_RUN_INTEGRATION=1 (kind=stub until an LF-driven dispatcher swap of `cubed_sphere_advection.jl` lands). dsc-0fd. |
| `lax_friedrichs_flux_cubed_sphere_eta` | `finite_volume` | `scheme` | §7 | `src/operators/flux_1d.jl` (`flux_1d` ArrayOp on CubedSphereGrid, dim=:eta branch); mirrored as `discretizations/finite_volume/lax_friedrichs_flux_cubed_sphere_eta.json` | P2 | atmos.dyn (reference / debugging on cubed sphere), atmos.chem.transport (oracle scheme) | same as `lax_friedrichs_flux_cubed_sphere_xi` | S | looks_ok | `discretizations/finite_volume/lax_friedrichs_flux_cubed_sphere_eta.json` | `lax_friedrichs_flux`, `cubed_sphere_panel_connectivity` | η-direction sibling of `lax_friedrichs_flux_cubed_sphere_xi`. Two-point stencil at offsets {-1, 0} along η at cell_center stagger; emits at v_edge. Closed-form F = (c+|c|)/2·q_south + (c-|c|)/2·q_north — coordinate-mirror image of the ξ rule under (xi, west, east) ↔ (eta, south, north). Layer-A canonical fixture is skip-only and points at the ξ-rule's hand-pinned fixture (the algebra is symmetric and the Cartesian-core unit tests pin the identity); Layer-B convergence is `applicable: false` (same gap as the ξ sibling). dsc-0fd. |
