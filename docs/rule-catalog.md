...
| `fv3_ppm_two_sided_edge` | `finite_volume` | `scheme` | §7 | Harris et al. (2021) Eq. 6.5–6.6; `src/operators/ppm_edge.jl` | P1 | atmos.dyn (FV3 advection at panel edges) | needs cross-panel ghost-cell access in stencil | M | looks_ok | `discretizations/finite_volume/fv3_ppm_two_sided_edge.esm` | `cam5_fv_ppm_reconstruction`, `fv3_cubed_sphere_corner_singularities` | Two-sided extrapolation for PPM at cube edges. |
...
| `lax_friedrichs_flux` | `finite_volume` | `scheme` | §7 | `src/operators/flux_1d.jl`, `transport_2d.jl` | P2 | atmos.dyn (reference / debugging) | none | S | needs_review | `discretizations/finite_volume/lax_friedrichs_flux.esm` | — | Existing repo uses LF as the default ArrayOp path; PPM is the higher-order numerical alternative. LF is dissipative — keep for testing only. |
