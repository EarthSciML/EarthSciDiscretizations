# EarthSciDiscretizations — Rule Catalog Roadmap (Hand-authored)

> **Status:** This file preserves the hand-authored Phase-0 catalog that
> predates the generator. Rules already committed under `discretizations/` are
> now manifested by the autogenerated [`rule-catalog.md`](rule-catalog.md)
> (produced by `docs/generate_rule_catalog.jl`). This roadmap retains the
> aspirational and source-survey content (CAM5 / FV3 / MPAS / HOMME /
> deep-dive) until a follow-up bead migrates it into a generated form. Do not
> treat this file as the source of truth for what is in the repo today — the
> autogenerated file is.
...
| `cam5_fv_ppm_reconstruction` | `finite_volume` | `scheme` | §7 | CAM5 SciDoc §4.1.3 (PPM); Colella & Woodward (1984) | P0 | atmos.dyn, atmos.chem.transport, ocean.dyn | none beyond MPAS-style polynomial-reconstruction | L | needs_review | `discretizations/finite_volume/ppm_reconstruction.esm` | `centered_2nd_uniform`, `colella_woodward_limiter` | Formerly in `src/operators/reconstruction.jl` (retired in the operator-porting campaign). Boundary interfaces used a low-order fallback (cell-mean) — flag in audit. |
...
| `fv3_cd_grid_transformation` | `finite_volume` | `scheme` | §7 | Harris et al. (2021) §5; `discretizations/finite_volume/fv3_d_to_c_{xi,eta}.json` (former `src/operators/wind_ops.jl`, ported in commit `f063346`) | P1 | atmos.dyn (FV3 momentum) | none | M | needs_review | `discretizations/finite_volume/fv3_cd_grid_transform.esm` | `fv3_d_grid_staggering`, `fv3_c_grid_staggering` | C↔D grid algorithm — converts D-grid covariant to C-grid contravariant for advection. |
...
| `fv3_lin_rood_advection` | `finite_volume` | `scheme` | extension | Lin & Rood (1996) MWR; Putman & Lin (2007) JCP; former `src/operators/transport_2d.jl` (retired) | P1 | atmos.chem.transport, atmos.dyn | needs dimensional-split rule type | XL | needs_review | `discretizations/finite_volume/lin_rood_advection.esm` | `cam5_fv_ppm_reconstruction`, `fv3_ppm_two_sided_edge` | Dimensionally-split 2D PPM advection on cubed sphere. The former `src/operators/transport_2d.jl` included a Lin-Rood path alongside Lax-Friedrichs (retired in the operator-porting campaign). |
| `fv_divergence_cubed_sphere` | `finite_volume` | `scheme` | §7 | former `src/operators/divergence.jl` (removed; see git history, commit `bb369ee`); cf. MPAS Skamarock §7.3 | P0 | atmos.dyn, ocean.dyn | none | S | looks_ok | `discretizations/finite_volume/fv_divergence_cubed_sphere.esm` | `fv3_cubed_sphere_grid`, `fv3_c_grid_staggering` | Julia builder retired (commit `bb369ee`). Maps directly to ESS RFC §7.3 divergence example. |
| `fv_gradient_cubed_sphere` | `finite_volume` | `scheme` | §7 | former `src/operators/gradient.jl` (removed; see git history, commit `bb369ee`) | P0 | atmos.dyn, ocean.dyn | none | S | looks_ok | `discretizations/finite_volume/fv_gradient_cubed_sphere.esm` | `fv3_cubed_sphere_grid` | Edge gradient using physical center-to-center distance. |
| `fv_laplacian_full_covariant_cubed_sphere` | `finite_volume` | `scheme` | §7 | former `src/operators/laplacian.jl` (retired) | P1 | atmos.dyn (diffusion on sphere) | needs cross-metric stencil composition | L | needs_review | `discretizations/finite_volume/fv_laplacian_full_covariant.esm` | `mixed_2nd_cross_derivative`, `fv3_cubed_sphere_grid` | Former 9-point covariant Laplacian (retired in the operator-porting campaign; math to be ported to a new rule). Cross-metric expansion is complex; needs careful audit. |
| `fv3_kinetic_energy` | `finite_volume` | `scheme` | extension | Harris et al. (2021) Eq. 6.3; `discretizations/finite_volume/fv3_kinetic_energy_cell.json` (former `src/operators/kinetic_energy.jl`, ported in commit `f063346`) | P1 | atmos.dyn (vector-invariant momentum) | needs covariant/contravariant pairing rule | L | needs_review | `discretizations/finite_volume/fv3_kinetic_energy.esm` | `fv3_d_grid_staggering`, `fv3_super_grid_sin_cos` | Upstream-biased KE on non-orthogonal grid. |
| `fv3_vorticity_corner_and_cellmean` | `finite_volume` | `scheme` | §7 | Harris et al. (2021) §3,5,6; `discretizations/finite_volume/fv3_vorticity_{corner,cellmean}.json` (former `src/operators/vorticity.jl`, ported in commit `f063346`) | P1 | atmos.dyn (FV3 PV flux) | needs corner-staggered output | M | looks_ok | `discretizations/finite_volume/fv3_vorticity.esm` | `fv3_d_grid_staggering`, `fv3_cubed_sphere_grid` | Two variants ported as JSON rules: corner-point (interpolated) and cell-mean (Stokes' theorem). |
...
| `discretization_pipeline_arrayop` | `finite_volume` | `scheme` | extension | former `src/discretization.jl`, `src/equation_discretizer.jl` (removed; see git history, commit `953119a` — tests now drive the ESS pipeline) | P1 | (engine glue) | needs ArrayOp template type in §7 (matches MOL PR #531) | L | needs_review | (engine; may not need `.esm`) | `mol_array_discretization_strategy` | The removed ESD pipeline used ArrayOp directly. Verify the ESS approach converges with MOL's so we share schema. |
