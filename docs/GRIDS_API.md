...
| Python | `earthsci_discretizations.grids.<family>(**opts)` |
...
grid = CubedSphereGrid(48; R = 6.371e6, Ng = 3)
```

- Module path: `EarthSciDiscretizations.grids`, one function per family.
  The Julia declarative builders landed so far are `duo`, `mpas`,
  `cartesian`, `vertical`, `lat_lon`, and `arakawa`; a
  `grids.cubed_sphere` builder has not landed yet (Python / Rust / TS
  have it), so the cubed sphere is constructed directly via
  `CubedSphereGrid(Nc; R, Ng)` as above.
...
grid = earthsci_discretizations.grids.cubed_sphere(
    Nc=48,
    R=6.371e6,
    dtype="float64",
    ghosts=3,
...
- Module path: `earthsci_discretizations.grids`, one function per family.
...
- Returns an `earthsci_discretizations.grids.Grid` — see §3.2.
...
- `earthsci_discretizations.grids.Grid` is an `xarray.Dataset`-backed wrapper (subclass or
...
- **Primary (specified, not yet implemented)**: `Meshes.jl` and
  `ClimaCore.jl`.
  - Spec: cubed-sphere, lat-lon, and cartesian families expose a
    `Meshes.Mesh` view via `as_meshes(grid)`; cubed-sphere and lat-lon
    families expose a `ClimaCore.Spaces` view via `as_climacore(grid)`
    where applicable.
  - Status: only a cartesian `as_meshes` stub exists today
    (`src/grids/cartesian.jl`), and it unconditionally throws an
    `ArgumentError`; no `as_climacore` method exists anywhere, and
    `Project.toml` declares no Meshes.jl / ClimaCore.jl dependencies or
    package extensions.
- **Fallback**: plain-array geometry accessors. No heavy deps;
...
- ESD `docs/rule-catalog-roadmap.md` — hand-authored Phase 0 rule
  inventory (the autogenerated manifest `docs/rule-catalog.md` is
  produced by `docs/generate_rule_catalog.jl` and is gitignored)
