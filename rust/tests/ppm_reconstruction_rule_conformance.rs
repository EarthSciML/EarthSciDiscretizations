...
//!
//! 1. **`sub_stencil_cases`** — `PpmRule::from_json` parses the named
//!    sub-stencils to the offsets and coefficient floats the harness
//!    pins. This is the structural equivalent of the latlon rule's
//!    `coefficient_cases` (cf. `lat_lon_rule_conformance.rs`).
//! 2. **`output_kind_cases`** — `PpmRule::evaluate` dispatches on
//!    `OutputKind` and reproduces hand-pinned values for the
//!    right-edge interpolation (CW84 eq. 1.6) and the parabola
//!    endpoints.
//! 3. **`convergence_case`** — the canonical MMS sweep from
//!    `tests/fixtures/ppm_reconstruction/{input,expected}.esm`. The
//!    Rust evaluator is exercised end-to-end (sub_stencil → output_kind
//!    → parabola) on the same smooth-periodic profile the Julia
//!    reference uses, and the measured minimum convergence order must
//!    clear `expected_min_order`. This is the "matching Julia
//!    evaluator on the canonical PPM fixture" acceptance criterion.

use std::fs;
use std::path::{Path, PathBuf};

use earthsci_grids::ppm_reconstruction::{OutputKind, PpmRule, SubStencil};
...
fn parse_sub_stencil(s: &str) -> SubStencil {
    match s {
        "q_left_edge" => SubStencil::QLeftEdge,
        "q_right_edge" => SubStencil::QRightEdge,
...
fn ppm_rule_evals_match_conformance_fixture() {
    ...
    let rule = PpmRule::from_json(&rule_value).expect("rule parses");

    // (1) sub_stencil structural cases: offsets + coefficient floats.
    for case in spec["sub_stencil_cases"].as_array().unwrap() {
        let name = case["name"].as_str().unwrap();
        let sub = parse_sub_stencil(case["sub_stencil"].as_str().unwrap());
        let entries = rule.entries(sub);
    ...
    for case in spec["output_kind_cases"].as_array().unwrap() {
        ...
        match kind {
            "q_right_edge" | "q_left_edge" => {
                ...
                let output = if kind == "q_right_edge" {
                    OutputKind::QRightEdge
                } else {
                    OutputKind::QLeftEdge
                };
                let got = rule.evaluate(output, q_at).unwrap();
            ...
            "q_parabola" => {
                for sample in case["samples"].as_array().unwrap() {
                    ...
                    let got = earthsci_grids::ppm_reconstruction::parabola(a_l, a_r, q_i, xi);
                    assert!(
                        close_rel(got, expected, rel_tol),
    ...
    let linf_error = |n: usize| -> f64 {
        ...
        for i in 0..n {
            ...
            for k in 1..=samples_per_cell {
                ...
                let recon = rule
                    .evaluate(OutputKind::QParabola { xi }, q_at)
                    .expect("parabola eval");
