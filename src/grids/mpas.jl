...
Fully declarative (esd-heg.9): the imperative dual construction is retired — the
dual TOPOLOGY (angular circumcenter ring, bridge-vertex neighbours, incident edges)
comes from `voronoi_dual_topology_faq` (D1b, src/topology_faq.jl) and the dual
GEOMETRY (circumcenters, cell/edge geographic coords, dc/dv edge arcs, dual-cell
areas) from `duo_dual_geometry_faq` (D2b, src/edge_dual_geometry_faq.jl), both
evaluated through the landed ESS engine. Byte-identical (T=Float64) to the prior
imperative output — pinned by the dual topology/geometry conformance goldens.
"""
function _duo_voronoi_dual(level::Int; R::Real = 6.371e6)
    ...
    # ---- Dual TOPOLOGY (D1b value-invention FAQ) ----
    topo = voronoi_dual_topology_faq(duo.vertices, duo.faces, duo.cell_cart; R = R_f)

    # ---- Dual GEOMETRY (D2b geometry FAQ over the angular ring) ----
    edge_cells = _duo_edge_cells(duo)   # (2, Ne) the two incident DUO faces per edge
    geo = duo_dual_geometry_faq(
        Float64, duo.vertices, duo.faces, duo.cell_cart,
        duo.edges, edge_cells, topo.sorted_vertex_faces, R_f,
    )

    # MPAS edge e ↔ DUO edge e: the two MPAS cells are the DUO edge's vertex pair.
    cells_on_edge = Matrix{Int}(undef, 2, Ne)
    @inbounds for e in 1:Ne
        cells_on_edge[1, e] = duo.edges[1, e]
        cells_on_edge[2, e] = duo.edges[2, e]
    end
    ...
    return mpas_mesh_data(;
        ...
        n_edges_on_cell = topo.n_edges_on_cell,
        cells_on_cell = topo.cells_on_cell,
        edges_on_cell = topo.edges_on_cell,
        lon_edge = geo.lon_edge,
        lat_edge = geo.lat_edge,
        cells_on_edge = cells_on_edge,
        dc_edge = geo.dc_edge,
        dv_edge = geo.dv_edge,
        max_edges = topo.max_edges,
        n_vertices = Nc,
        R = R_f,
    )
end
