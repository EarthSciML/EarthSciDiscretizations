...
function _cartesian(; nx::Union{Int,Nothing}=nothing,
                      ny::Union{Int,Nothing}=nothing,
                      nz::Union{Int,Nothing}=nothing,
                      extent=nothing,
                      edges=nothing,
                      dtype::Type{T}=Float64,
                      ghosts::Int=0) where {T<:AbstractFloat}
...
function metric_eval(grid::CartesianGrid{T,N}, name::Symbol, idx::Vararg{Int,N}) where {T,N}
    ...
    # axis widths
    if name === :dx; _check_axis(N, 1); return grid.widths[1][idx[1]]; end
    if name === :dy; _check_axis(N, 2); return grid.widths[2][idx[2]]; end
    if name === :dz; _check_axis(N, 3); return grid.widths[3][idx[3]]; end
    if name === :face_area_x; return _face_area(grid, 1, idx); end
    if name === :face_area_y; _check_axis(N, 2); return _face_area(grid, 2, idx); end
    if name === :face_area_z; _check_axis(N, 3); return _face_area(grid, 3, idx); end
