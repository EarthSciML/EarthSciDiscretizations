...
function wrap_esm(N::Int, rhs; extra_vars=Dict{String,Any}(), rules=nothing)
    ...
    d = Dict("esm"=>"0.4.0","metadata"=>Dict("name"=>"integral_probe"),
...
function check(label, input, N, expect; const_arrays=Dict{String,AbstractArray{Float64}}())
    ...
    out = try ESM.discretize(input; lift_1d_arrayop=true)
          catch e; println("discretize THREW: ", first(split(sprint(showerror,e), '\n'))); return end
    try
        ...
        println(err<1e-10 ? "PASS  du≡$(expect)  (Linf err=$err)" : "WRONG (err=$err, vals[1]=$(vals[1]))")
    catch e; println("eval THREW: ", first(split(sprint(showerror,e), '\n'))) end
end

# --- Check 1: ESS `integral` op (existing host quadrature, grid-agnostic) ---
function integral_op(N)
    integral = Dict("op"=>"integral","args"=>["u"],"var"=>"x",
                    "lower"=>Dict("op"=>"const","value"=>0.0),"upper"=>Dict("op"=>"const","value"=>1.0))
    wrap_esm(N, Dict("op"=>"neg","args"=>[integral]);
...
function arrayop_integral(N, reduce_hi)
    ...
    wrap_esm(N, Dict("op"=>"neg","args"=>[repl]);
...
function const_sized_integral(N)
    ...
    repl = Dict("op"=>"arrayop","reduce"=>"+","ranges"=>Dict("j"=>[1, idx("size_x",1)]),
...
function rule_path_integral(N)
    integral = Dict("op"=>"integral","args"=>["u"],"var"=>"x",
                    "lower"=>Dict("op"=>"const","value"=>0.0),"upper"=>Dict("op"=>"const","value"=>1.0))
    repl = Dict("op"=>"*","args"=>["dx",
        Dict("op"=>"arrayop","reduce"=>"+","ranges"=>Dict("j"=>[1, idx("size_x",1)]),
             "expr"=>idx("u","j"), "args"=>["size_x"])])
    rule = Dict("name"=>"midpoint_1d",
        "pattern"=>Dict("op"=>"integral","args"=>["\$u"],"var"=>"x",
                        "lower"=>Dict("op"=>"const","value"=>0.0),"upper"=>Dict("op"=>"const","value"=>1.0)),
        "replacement"=>repl)
    wrap_esm(N, Dict("op"=>"neg","args"=>[integral]);
...
for N in (8,16,32); check("1: integral OP      N=$N", integral_op(N), N, -1.0); end
println("--- Check 2: scalar reduce-arrayop integral, CONCRETE bound (mechanism) ---")
for N in (8,16); check("2: arrayop concrete  N=$N", arrayop_integral(N,N), N, -1.0); end
...
for N in (8,16,32)
    check("4: const-array bound N=$N", const_sized_integral(N), N, -1.0;
...
for N in (8,16,32)
    check("5: rule path        N=$N", rule_path_integral(N), N, -1.0;
...
for N in (8,16)
    ...
    for i in 1:N; supp[i,1]=mod1(i-1,N); supp[i,2]=i; supp[i,3]=mod1(i+1,N); end
    check("6: compact OP       N=$N", compact_integral(N), N, -3.0*h;
