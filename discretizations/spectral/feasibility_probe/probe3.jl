...
_red(body,i,ilo,ihi,j,jlo,jhi)=OpExpr("arrayop",ESM.Expr[];output_idx=Any[i],expr_body=body,
    ranges=Dict(i=>[ilo,ihi],j=>[jlo,jhi]),reduce="+")
probe(f,name)=(print("[$name] "); try f() catch e; println("THREW: ",sprint(showerror,e)) end)
...
probe("a chebyshev-1st-deriv") do
    ...
    diag = _op("ifelse", _op("==",_v("i"),_n(1)), corner,
    ...
    println(err<1e-8 ? "PASS (Linf err=$err vs 3x^2, exact poly)" :
...
probe("b fourier-N16-wavenumber2") do
    ...
    err=maximum(abs.(vals.-expect))
    println(err<1e-6 ? "PASS (Linf err=$err vs 2cos(2x))" : "WRONG (err=$err)")
end
