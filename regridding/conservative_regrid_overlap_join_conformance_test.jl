...
#
# ESD owns this declarative regridder PROGRAM (conservative_regrid_overlap_join.esm,
# co-located here); the EarthSciSerialization engine (an ESD dependency) EVALUATES
# it. Relocated from ESS (bead esd-9cb / F4) — the ESS engine + geometry PRIMITIVES
# (intersect_polygon / polygon_area, GeometryOps-backed) stay in ESS; this test
# reaches them through the dependency. Mirrors the ESD precedent
# discretizations/finite_volume/vertical_remap.json (a declarative grid-remap owned
# by ESD, evaluated via the ESS engine).
#
# RFC semiring-faq-unified-ir §A.8 / §8.1 / §6.1 / §5.3 / §5.5; CONFORMANCE_SPEC.md
# §5.8. Drives the FULL conservative-regridding idiom in ONE document —
# conservative_regrid_overlap_join.esm — through build_evaluator, with NO pre-baked
# bins:
#
...
@testsnippet RegridOverlapJoinSetup begin
    using Test
    import EarthSciSerialization as ESS
    # Trigger the EarthSciSerializationGeometryOpsExt extension: the spherical
    # intersect_polygon clip is GeometryOps-backed (a weakdep of the engine), so
    # loading GeometryOps + GeoInterface in the test session activates it.
    ...
    # the raw document is the exact type the value-invention front-door expects.
    const JSON3 = ESS.JSON3

    const _OJ_FIXTURE = joinpath(@__DIR__, "conservative_regrid_overlap_join.esm")
